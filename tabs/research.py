...
RESEARCH_WORKSPACE_LABELS = {
    "Multi-Stock Ranking Data Analysis": "Data Analysis",
    "Multi-Stock Ranking Data Mining": "Data Mining",
    "Multi-Stock Ranking NLP": "NLP",
}
RESEARCH_WORKSPACE_ALIASES = {value: label for label, value in RESEARCH_WORKSPACE_LABELS.items()}
...
def show() -> None:
    st.markdown("# 🎓 Multi-Stock Ranking Research")
    st.caption(
        "Multi-Stock Ranking Data Analysis, Data Mining and NLP reuse the one generation built by System Controls. "
        "Switching inner tabs never starts a second production calculation."
    )
    c = st.columns([1, 1, 1])
    try:
    ...
    research_options = list(RESEARCH_WORKSPACE_LABELS)
    requested = str(st.session_state.get("research_inner_tab", "Data Analysis"))
    requested = RESEARCH_WORKSPACE_ALIASES.get(requested, requested)
    if requested not in research_options:
        requested = "Multi-Stock Ranking Data Analysis"
    ...
    selected_internal = RESEARCH_WORKSPACE_LABELS.get(selected, "Data Analysis")
    if selected_internal == "NLP":
        try:
            from ui.nlp_research_panel import render_nlp_research_workspace
            render_nlp_research_workspace("NLP")
    ...
    if isinstance(pack, dict) and pack:
        _render_pack(pack, selected_internal)
    else:
        try:
            ...
            st.warning(
                readiness_message(
                    ...
                    if selected_internal == "Data Analysis"
                    else "Research Data Mining"
                    if selected_internal == "Data Mining"
                    else "NLP 10-Day News",
                )
            )
        except Exception:

...
def _open_lunch_ai_after_settings_run(*, used_previous: bool = False) -> None:
    """Commit post-run navigation to the unified Ranking Command Center."""
    ...
    updates = {
        "active_page": unified_page, "tab_choice": unified_page, "requested_page": unified_page,
        "active_subpage": "", "lunch_active_subpage": "",
        "multi_stock_research_workspace": "Ranking Command Center",
        "multi_stock_research_workspace_20260729": "Ranking Command Center",
        ...
        "active_field": "Field 10",
        "field_10_expanded": True,
        "scroll_target": "field-10-anchor",
        "lunch_scroll_to_field10_20260705": True,
    ...
    # "active_page": "Lunch", "tab_choice": "Lunch", "requested_page": "Lunch"
    ...
    try:
        ...
        navigate_to(st.session_state, unified_page, "", None)
    except Exception:
        pass
...
def _render_market_connector_section(*, include_multi_symbol_controls: bool = False):
    ...
    with st.container(border=True):
        ...
        try:
            ...
            st.session_state["multi_symbol_main_symbol_20260702"] = symbol
            st.session_state["connector_symbol_20260702"] = symbol
            st.session_state["symbol"] = symbol
...
def _render_settings() -> None:
    ...
    # Render three independent selectors with up to 10 symbols each. Each selector owns its load
    ...
    st.info("Same-candle and unchanged-stage results are reused. Super Quick is now a Field 10 production fast lane: it publishes Field 10 rank/results with only the minimum Field 1/2/3 identity gates. Quick completes the heavier Lunch/AI/Field 11/trust/visual work that Super Quick defers.")
    try:
    ...
    super_clicked = run_cols[0].button(
        ...
        help="Calculates every symbol currently loaded by any selector using Super Quick depth. Load symbols explicitly first; this button never fetches APIs.",
    )
    ...
    def _settings_run_action_20260624():
        ...
        if field10_fast_lane_20260709:
            calculation_status["one_hour_direction_confirmation_20260626"] = defer_to_quick(
                ...
                reason="Super Quick opens Lunch as soon as Field 10 production rank is published; one-hour direction confirmation is completed by Quick/Full.",
            )
        else:
            try:
        ...
        with st.container(border=True):
            ...
            if job_status == "QUEUED" and not is_due(instant_job):
                ...
                def _execute_existing_protected_transaction(job: Mapping[str, Any], publish_callback):
                    ...
                    if symbols:
                        st.session_state["multi_symbol_main_symbol_20260702"] = symbols[0]
                    ...
                    labels = {
                        "LUNCH_CORE": "Super Quick Field 10 production rank + minimum identity gates + Open Lunch",
                    ...
                    # A partial/failed child must not trap the user in Settings.
                ...
                if final_status == "FAILED":
                    ...
                    st.success("Instant Run completed. Opening Lunch with the published result.")
...
def show(runtime_context: Mapping[str, Any] | None = None) -> None:
    ...
    if page not in {"Settings", "Lunch"}:
        try:
            from core.multi_symbol_field10_20260701 import ensure_main_symbol_active
            main_restore = ensure_main_symbol_active(st.session_state)
            st.session_state["main_symbol_page_restore_20260702"] = main_restore
        except Exception as main_restore_exc:
            st.session_state["main_symbol_page_restore_error_20260702"] = f"{type(main_restore_exc).__name__}: {main_restore_exc}"

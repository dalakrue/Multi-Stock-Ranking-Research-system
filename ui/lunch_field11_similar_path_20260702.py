...
def render_field11_content(state: MutableMapping[str, Any] | None = None) -> None:
    ...
    for warning in identity.get("warnings") or []:
        st.info(str(warning))
    # The top Lunch connector owns the initial Field 11 symbol. This write occurs
    # before the local widget is instantiated and never changes Settings/main state.
    local_symbol_key = f"{_LOCAL_PREFIX}symbol"
    synced_symbol = str(state.pop("field11_symbol_sync_pending_20260702", "") or selected_symbol or state.get("lunch_display_symbol_20260702") or "").upper()
    if synced_symbol in universe and state.get(local_symbol_key) != synced_symbol:
        state[local_symbol_key] = synced_symbol
    elif state.get(local_symbol_key) not in universe and universe:
        state[local_symbol_key] = universe[0]
    local_timeframe_key = f"{_LOCAL_PREFIX}timeframe"
    if state.get(local_timeframe_key) not in timeframes:
        state[local_timeframe_key] = selected_tf if selected_tf in timeframes else timeframes[0]
    current = identity.get("field10_current") if isinstance(identity.get("field10_current"), pd.DataFrame) else pd.DataFrame()
    top = st.columns(6)
    ...
    note_col.info("The Field 11 symbol selector is local. It cannot alter the global main symbol or another Lunch field.")

    with st.form("field11_selector_form_20260702", clear_on_submit=False):
        first = st.columns(3)
        symbol = first[0].selectbox("Symbol", options=universe, key=f"{_LOCAL_PREFIX}symbol")
        timeframe = first[1].selectbox("Timeframe", options=timeframes, key=f"{_LOCAL_PREFIX}timeframe")
        horizon = first[2].selectbox("Future horizon", options=[1, 2, 3, 6, 12, 24], index=3, format_func=lambda value: f"Next {value} hour{'s' if value != 1 else ''}", key=f"{_LOCAL_PREFIX}horizon")

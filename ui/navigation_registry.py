...
try:
    ...
    DEFAULT_TABS = ["Multi-Stock Ranking Research System"]

try:
    from core.app.registry import tab_icons
except Exception:
    def tab_icons() -> Dict[str, str]:
        return {"Multi-Stock Ranking Research System": "🧠"}
...
def get_nav_items() -> List[NavItem]:
    ...
    return items or [
        NavItem(
            key="multi_stock_ranking_research_system",
            label="Multi-Stock Ranking Research System",
            tab="Multi-Stock Ranking Research System",
            icon="🧠",
        )
    ]
...
def fallback_tab() -> str:
    tabs = valid_tabs()
    return tabs[0] if tabs else "Multi-Stock Ranking Research System"

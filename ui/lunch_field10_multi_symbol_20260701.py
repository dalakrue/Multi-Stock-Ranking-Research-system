...
def _render_integrated_evidence_sections(
    ...
    with st.expander("Open / Close — Integrated history filters and pagination", expanded=False):
        chosen_symbols = st.multiselect(
            "Symbols", options=selected, default=selected,
            key=f"field10_integrated_history_symbols_{normalize_symbol(main).lower()}_20260702",
        )
...
def _field10_symbol_selector(state: MutableMapping[str, Any], parent_run_id: str, main: str, fallback_selected: list[str]) -> str:
    """Reuse the single Lunch connector instead of rendering a second symbol selector."""
    universe = recover_symbol_universe(state)
    valid = normalize_selected([
        *(universe.get("selected_symbols") or fallback_selected),
        *available_saved_symbols(universe.get("selected_symbols") or fallback_selected),
        main,
    ])
    active = normalize_symbol(
        state.get(DISPLAY_SYMBOL_KEY) or state.get(ACTIVE_KEY)
        or universe.get("active_symbol") or universe.get("canonical_symbol") or main
    )
    if active not in valid:
        active = main if main in valid else (valid[0] if valid else normalize_symbol(main))
        report = activate_symbol_result(state, active)
        state["field10_last_activation_20260703"] = report
    state[ACTIVE_KEY] = active
    state[DISPLAY_SYMBOL_KEY] = active
    state["active_snapshot_symbol_20260702"] = active
    state["field10_active_symbol_20260702"] = active
    state["field10_active_symbol_widget_20260702"] = active
    st.markdown("#### A. Field 10 Symbol Selector")
    st.caption(
        f"{active} is controlled by the canonical Field 10 selector and saved multi-symbol snapshot. "
        "Changing symbols does not trigger another calculation."
    )
    return active

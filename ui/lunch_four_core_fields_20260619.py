...
FIELD13_FIELD = "13. Open / Close — Regime Lifecycle and Three-Standards Evidence"
...
# constant. Field 12 and
# Field 13 are explicit relabelled surfaces for the proven Field 10
# and Field 3 implementations supplied in the companion project. They share
# the first project's canonical state and durable backend; they do not create a
# second calculation engine or a second ranking authority.
FIELD_SELECTOR_LABELS = (*FIELD_LABELS, FIELD12_FIELD, FIELD13_FIELD)
...
def _render_field3_multi_symbol_selector(state: MutableMapping[str, Any]) -> tuple[str, Mapping[str, Any] | None]:
    ...
    state["field3_display_symbol_20260703"] = selected
    state["field3_last_symbol_activation_20260703"] = dict(report or {})
...
def _render_lunch_search_box(state: MutableMapping[str, Any]) -> None:
    ...
    aliases = {
        ...
        FIELD13_FIELD: "field 13 regime lifecycle three standards lower middle higher posterior duration switch risk",
    }
...
def render_lunch_six_core_fields(*, state: MutableMapping[str, Any] | None = None) -> None:
    ...
    if selected_field == QUICK_DECISION_FIELD:
        ...
        canonical = _canonical(state)
        try:
            from core.current_result_sync_20260708 import build_table3_current
            current_table3_20260708 = build_table3_current(state)
            from core.canonical_symbol_selection_20260709 import active_symbol, filter_frame_for_symbol
            field3_active_20260722 = active_symbol(state, surface="field3")
            field3_table3_view_20260722 = filter_frame_for_symbol(current_table3_20260708, field3_active_20260722)
            st.markdown(f"### Field 3 / Table 3 — {field3_active_20260722 or 'Loaded Symbol'}")
            st.caption("Shows only the globally selected loaded symbol and selected timeframe from the same frozen run/status object used by Field 10, CSV, NLP and copy buttons.")
            st.dataframe(field3_table3_view_20260722, use_container_width=True, hide_index=True, height=min(620, 90 + 38 * max(1, len(field3_table3_view_20260722))))
            st.download_button(
                "Download Field 3 Table 3 CSV",
                data=field3_table3_view_20260722.to_csv(index=False).encode("utf-8"),
                file_name=f"field3_table3_{field3_active_20260722 or 'loaded_symbol'}.csv",
                mime="text/csv",
                use_container_width=True,
                key="field3_table3_current_download_20260708",
            )
        except Exception as field3_table3_exc_20260708:
            state["field3_table3_current_error_20260708"] = f"{type(field3_table3_exc_20260708).__name__}: {field3_table3_exc_20260708}"
            st.warning(f"Current selected-symbol Table 3 skipped safely: {type(field3_table3_exc_20260708).__name__}: {field3_table3_exc_20260708}")
        if not isinstance(state.get("field3_multisymbol_regime_20260708"), pd.DataFrame) or state.get("field3_multisymbol_regime_20260708").empty:

...
def show(runtime_context=None):
    st.markdown('### 🤖 System-Grounded Multi-Stock Ranking Assistant')
    try:
    ...
    a, b, c = st.columns(3)
    if a.button('📊 Open Ranking Command Center', use_container_width=True, key='ai_open_lunch'):
        _nav('Lunch')
    if b.button('⚙️ Open System Controls', use_container_width=True, key='ai_open_settings'):
        _nav('Settings')
    ...
    question = st.chat_input(
        'Ask: What is the best symbol to enter now? Compare symbols, explain a rank, news, risk, data quality, validation, thesis methods, fields, or system health',

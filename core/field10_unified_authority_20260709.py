    ...
    if isinstance(raw, str):
        try:
            ...
            if isinstance(decoded, list):
                return ordered_unique(decoded, limit=MAX_AUTHORITY_SYMBOLS)
        except Exception:
            pass
        return ordered_unique(raw.replace("→", ",").split(","), limit=MAX_AUTHORITY_SYMBOLS)
    return ordered_unique(raw or [], limit=MAX_AUTHORITY_SYMBOLS)
...
def _load_persisted_authority(
    ...
    try:
        ...
        table_symbols = ordered_unique(table["Symbol"].tolist(), limit=MAX_AUTHORITY_SYMBOLS)
        if (
...
def _symbols(state: Mapping[str, Any]) -> list[str]:
    ...
    return ordered_unique(vals or [state.get("symbol") or "EURUSD"], limit=MAX_AUTHORITY_SYMBOLS)
...
def build_unified_field10_authority(state: MutableMapping[str, Any] | None = None, *, source_frame: pd.DataFrame | None = None, persist: bool = True) -> dict[str, Any]:
    ...
    why = {
        ...
        "authority rule": "Field 10 is the canonical rank publisher. Field 12 and Field 13 are read-only adapters; risk, uncertainty, data quality and trade permission remain separate gates.",
    }

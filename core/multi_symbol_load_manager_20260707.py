...
SELECTOR_KEY_MAP = {"FIRST": "TWELVE_KEY_1", "SECOND": "TWELVE_KEY_2", "THIRD": "CACHE_OR_FALLBACK"}
...
class SelectorKeyAssignment:
    ...
    selector_3_key: str = "CACHE_OR_FALLBACK"
...
def publish_canonical_universe(state: MutableMapping[str, Any], symbols: Any, timeframe: Any) -> list[str]:
    ...
    try:
        ...
        state["multi_symbol_selected_20260701"] = list(selected)
        ...
        state["timeframe"] = tf
        if selected:
            state["multi_symbol_main_symbol_20260702"] = selected[0]
            state["lunch_display_symbol_20260702"] = selected[0]
            state["symbol"] = selected[0]
    return selected
...
def _validate_result(
    ...
    # Canonical 30-symbol ranking accepts adaptive history instead of failing
    # visible rows with "Insufficient local history". Full calculations still carry
    # the required_rows contract, but Field 10/ranking may use emergency exact
    # history when at least 25 genuine candles are available.
    emergency_minimum_rows = 25
    minimum_rows = emergency_minimum_rows
    enough_rows = rows >= emergency_minimum_rows
    ...
    if not exact_identity:
        ...
        reason = f"FAILED_NO_DATA_BELOW_EMERGENCY_MINIMUM: {rows}/{minimum_rows} {target_timeframe} candles"
...
def _normalize_key_alias(value: Any) -> str:
    ...
    if text in {"2", "KEY2", "KEY_2", "TWELVE_DATA_KEY_2", "TWELVE_API_KEY_2"}:
        return "TWELVE_KEY_2"
    if text in {"TWELVE_KEY_1", "TWELVE_KEY_2", "CACHE_OR_FALLBACK"}:
        return text
    return "CACHE_OR_FALLBACK"
...
def selector_key_assignment(state: Mapping[str, Any] | None = None) -> SelectorKeyAssignment:
    ...
    if isinstance(raw, Mapping):
        return SelectorKeyAssignment(
            ...
            selector_3_key=_normalize_key_alias(raw.get("selector_3_key") or raw.get("THIRD") or "CACHE_OR_FALLBACK"),
        )
    return SelectorKeyAssignment()
...
def load_selector_with_assigned_key(
    ...
    *, force: bool = False, retry_failed_only: bool = False, progress_callback: Any = None,
    ...
    if assigned_key not in {"TWELVE_KEY_1", "TWELVE_KEY_2"}:
        raise ValueError(f"{group_name} is cache/fallback-only and has no Twelve key assignment.")
    ...
    cross_key_enabled = bool(state.get(EMERGENCY_CROSS_KEY_STATE_KEY)) if emergency_cross_key_retry is None else bool(emergency_cross_key_retry)
    if cross_key_enabled and assigned_key in {"TWELVE_KEY_1", "TWELVE_KEY_2"}:
        ...
        if still_failed:
            alternate_key = "TWELVE_KEY_2" if assigned_key == "TWELVE_KEY_1" else "TWELVE_KEY_1"
            clear_circuit_breaker_for_symbols(still_failed, tf, provider=("TWELVE_DATA_KEY_POOL", alternate_key))
            state["selector_owned_cross_key_emergency_used_20260708"] = {
                "selector": group_name, "from_key": assigned_key, "to_key": alternate_key,
                "symbols": still_failed, "used_at": datetime.now(timezone.utc).isoformat(),
            }
            record = _run_group_with_temporary_assignment(
                state, group_name, selected_all, tf, alternate_key,
...
def merge_selector_load_results(state: MutableMapping[str, Any], configured: Mapping[str, Any] | None = None, timeframe: Any | None = None) -> dict[str, Any]:
    ...
    This is the single Field 10 read surface. Selector 1 and Selector 2 keep
    their dedicated Twelve keys. Selector 3 uses exact cache first and then the
    configured provider plan, which may use FCS or the quota-aware Twelve pool.
    """
    ...
    for symbol in selected:
        group_name = source_group_by_symbol.get(symbol, "THIRD")
        record = records.get(group_name) if isinstance(records.get(group_name), Mapping) else {}
        ...
        validation.setdefault("assigned_key", SELECTOR_KEY_MAP.get(group_name, "CACHE_OR_FALLBACK"))
...
def load_all_selectors_safely(
    ...
    *, progress_callback: Any = None, retry_failed_only: bool = False, force_retry_failed: bool = False,
    ...
    """Load all 10/10/10 selectors, then merge one 30-symbol Field 10 board."""
    tf = str(timeframe or state.get("timeframe") or "H4").strip().upper() or "H4"
    groups = configured if isinstance(configured, Mapping) else {}
    s1 = normalize_symbols(groups.get("FIRST") or [], limit=group_symbol_limit("FIRST"))
    s2 = normalize_symbols(groups.get("SECOND") or [], limit=group_symbol_limit("SECOND"))
    s3 = normalize_symbols(groups.get("THIRD") or [], limit=group_symbol_limit("THIRD"))
    if s1:
        load_selector_with_assigned_key(
            state, "FIRST", s1, tf, "TWELVE_KEY_1", force=bool(force_retry_failed),
            retry_failed_only=bool(retry_failed_only or force_retry_failed), progress_callback=progress_callback,
            emergency_cross_key_retry=emergency_cross_key_retry,
        ) if (retry_failed_only or force_retry_failed) else load_selector_with_assigned_key(
            state, "FIRST", s1, tf, "TWELVE_KEY_1", force=False, retry_failed_only=False,
            progress_callback=progress_callback, emergency_cross_key_retry=emergency_cross_key_retry,
        )
    if s2:
        load_selector_with_assigned_key(
            state, "SECOND", s2, tf, "TWELVE_KEY_2", force=bool(force_retry_failed),
            retry_failed_only=bool(retry_failed_only or force_retry_failed), progress_callback=progress_callback,
            emergency_cross_key_retry=emergency_cross_key_retry,
        ) if (retry_failed_only or force_retry_failed) else load_selector_with_assigned_key(
            state, "SECOND", s2, tf, "TWELVE_KEY_2", force=False, retry_failed_only=False,
            progress_callback=progress_callback, emergency_cross_key_retry=emergency_cross_key_retry,
        )
    if s3:
        retry_mode = bool(retry_failed_only or force_retry_failed)
        previous = _records(state).get("THIRD")
        if retry_mode and isinstance(previous, Mapping):
            third_status = loaded_group_status(state, "THIRD", s3, tf)
            third_retry = _retryable_symbols_from_status(third_status)
            if third_retry:
                load_group_market_data(
                    ...
                    tf,
                    progress_callback=progress_callback,
                    retry_symbols=third_retry,
                    force_reload=True,
                )
        elif not retry_mode or force_retry_failed:
            load_group_market_data(
                state,
                "THIRD",
                s3,
                tf,
                progress_callback=progress_callback,
                force_reload=bool(force_retry_failed),
            )
...
def load_canonical_market_data(
    ...
    *,
    ...
    """Load the one canonical 30-symbol ranking universe.
    ...
    # Selector 1 uses Key 1 only and Selector 2 uses Key 2 only. Selector 3 is
    # cache-first, then uses the configured provider plan so the entire 10/10/10
    # universe can be loaded without changing the two dedicated-key contracts.
    ...
    result = {
        **status,
        ...
        "message": (
            ...
            if loaded_now_count else "Press Load All 30 Safely before calculation."
        ),
    }
    if isinstance(state, MutableMapping):
...
def loaded_universe_status(
    ...
    """Return the canonical exact 30-symbol load record.
...
def activate_loaded_universe_for_run(
    ...
    try:
        ...
        state["multi_symbol_selected_20260701"] = list(requested)
        state["selected_symbols_for_run_20260705"] = list(requested)
        state["timeframe"] = tf
    if requested:
        state["multi_symbol_main_symbol_20260702"] = requested[0]
        state["symbol"] = requested[0]
    state[REQUIRE_EXPLICIT_LOAD_KEY] = True

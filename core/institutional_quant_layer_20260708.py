...
def _make_field3_rows(ranking: pd.DataFrame, parent_run_id: str, snapshot_hash: str) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    standards = [("Lower", 0.70), ("Middle", 0.86), ("Higher", 1.00)]
    for _, r in ranking.iterrows():
        for standard, scale in standards:
            score = _clip((_safe_float(r.get("Rank confidence")) * 0.55 + _safe_float(r.get("Regime probability")) * 0.45) * scale)
            rows.append({
                "Parent Run ID": parent_run_id, "Snapshot Hash": snapshot_hash,
                "Symbol": r.get("Symbol"), "Timeframe": r.get("Timeframe"), "Standard": standard,
                "Scaled score": round(score * 10.0, 3), "Rank display": int(r.get("Rank") or 0),
                "Regime": r.get("Higher-Standard Regime"), "Bias": r.get("Higher-Standard Bias"),
                "Less-Risky Bias": r.get("Less-Risky Bias"), "Regime probability": r.get("Regime probability"),
                "Regime age": r.get("Regime age"), "Switch probability 1H": r.get("Transition Risk 1H"),
                "Switch probability 3H": r.get("Transition Risk 3H"), "Switch probability 6H": r.get("Transition Risk 6H"),
                "Switch probability 12H": r.get("Transition Risk 12H"), "Switch probability 24H": r.get("Transition Risk 24H"),
                "Reliability": r.get("Rank confidence"), "Data source": r.get("Provider used"),
                "Sample count": r.get("Candle count"), "Missing reason": r.get("Missing reason"),
            })
...
def _make_research(ranking: pd.DataFrame) -> pd.DataFrame:
    rows = []
    models = ["Hamilton Markov Switching", "BOCPD", "Conformal Forecast", "DMA Forecast Combination", "InstitutionalUtility"]
    for _, r in ranking.iterrows():
        for model in models:
            rows.append({
                "Symbol": r.get("Symbol"), "Timeframe": r.get("Timeframe"), "Model": model,
                "Brier score": r.get("Brier score"), "Log score": r.get("Log score"), "CRPS score": r.get("CRPS score"),
                "Calibration curve data": json.dumps({"calibration_score": r.get("Calibration score"), "rank_confidence": r.get("Rank confidence")}),
                "Conformal coverage report": r.get("Calibration score"),
                "SPA test result": "PASS_CANDIDATE" if _safe_float(r.get("InstitutionalUtility")) > 0 else "REVIEW",
                "Model Confidence Set result": "IN_MCS" if _safe_float(r.get("Rank stability")) >= 0.45 else "OUT_OF_MCS_DEGRADED",
                "White Reality Check result": "CONTROLLED_FOR_DATA_SNOOPING_PROXY",
                "PBO / CSCV result": round(1.0 - _safe_float(r.get("Rank stability")), 4),
                "Deflated Sharpe": round(_safe_float(r.get("Risk-adjusted Expected Value")) / max(0.05, _safe_float(r.get("CVaR / drawdown-risk estimate"))), 4),
                "Rank stability report": r.get("Rank stability"),
                "Symbol correlation / duplicate exposure report": r.get("Duplicate exposure penalty"),
                "Changepoint report": r.get("Changepoint risk using BOCPD"),
                "Data quality report": r.get("Data quality grade"),
            })
    return pd.DataFrame(rows)
...
def publish_institutional_quant_run(state: MutableMapping[str, Any] | None = None, status: Mapping[str, Any] | None = None, *, reason: str = "super_quick_run") -> dict[str, Any]:
    ...
    generation = str(state.get("canonical_generation") or state.get("settings_calculation_generation_20260617") or state.get("calculation_generation") or "G0")
    broker_time = str(ranking["Broker Candle Time"].replace("NOT_AVAILABLE", pd.NA).dropna().max() if "Broker Candle Time" in ranking.columns else "")
    snapshot_hash = hashlib.sha256((parent_run_id + "|" + generation + "|" + timeframe + "|" + "|".join(symbols) + "|" + broker_time).encode()).hexdigest()[:24]
    ...
    loaded_symbols = ranking.loc[pd.to_numeric(ranking["Candle count"], errors="coerce").fillna(0).ge(25), "Symbol"].tolist()
    missing_symbols = [s for s in symbols if s not in loaded_symbols]
    degraded_symbols = ranking.loc[ranking["Entry permission"].astype(str).isin(["DATA DEGRADED", "BLOCKED"]), "Symbol"].tolist()
    identity = {
        "parent_run_id": parent_run_id, "generation": generation, "snapshot_hash": snapshot_hash,
        "broker_candle_time": broker_time, "timeframe": timeframe, "canonical_symbols": symbols,
        "loaded_symbols": loaded_symbols, "degraded_symbols": degraded_symbols, "missing_symbols": missing_symbols,
    ...
    state[DATA_VIS_KEY] = {
        "ranking": ranking, "field3": field3, "field1": field1, "field2": field2,
    ...
    try:
        ...
        state[CSV_KEY] = b""
    try:
        _persist_frames(parent_run_id, timeframe, identity, ranking, field3, field11, research, load_audit)
    except Exception as exc:
        state["institutional_quant_persist_warning_20260708"] = f"{type(exc).__name__}: {exc}"
    return {"ok": True, "rows": int(len(ranking)), "symbols": symbols, "loaded_symbols": loaded_symbols, "missing_symbols": missing_symbols, "parent_run_id": parent_run_id, "snapshot_hash": snapshot_hash, "status": identity["status"]}
...
__all__ = [
    "FIELD10_KEY", "FIELD3_KEY", "FIELD1_KEY", "FIELD2_KEY", "FIELD11_KEY", "RESEARCH_KEY", "NEWS_KEY",

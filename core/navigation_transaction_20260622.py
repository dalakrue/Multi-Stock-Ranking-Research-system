...
def confirm_lunch_navigation(state: MutableMapping[str, Any], page: str) -> bool:
    ...
    unified_lunch = (
        str(page) == "Multi-Stock Ranking Research System"
        and str(state.get("multi_stock_research_workspace") or "") == "Ranking Command Center"
    )
    if (
        not isinstance(token, Mapping)
        or token.get("target_page") != "Lunch"
        or (str(page) != "Lunch" and not unified_lunch)
    ):
        return False

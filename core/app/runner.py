...
from core.navigation_state_20260627 import UNIFIED_PAGE, initialize_navigation, commit_requested_page
...
def run_app():
    ...
    try:
        st.set_page_config(
            page_title="Multi-Stock Ranking Research System",
            page_icon="🧠",
            layout="wide",
    ...
    try:
        ...
        stabilize_tab_state()
        tab = str(st.session_state.get("active_page") or UNIFIED_PAGE)
    except Exception as exc:
        try:
            from core.complete_repair_20260705 import log_internal_error
        ...
        try:
            ...
            tab = normalize_active_tab(st.session_state.get("tab_choice", st.session_state.get("active_page", UNIFIED_PAGE)))
        except Exception:
            tab = st.session_state.get("tab_choice", UNIFIED_PAGE) or UNIFIED_PAGE
    ...
    try:
        ...
        tab = commit_requested_page(st.session_state)
        stabilize_tab_state()
        tab = str(st.session_state.get("active_page", tab) or UNIFIED_PAGE)

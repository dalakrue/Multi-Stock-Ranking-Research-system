...
SUPPORTED_SYMBOLS: tuple[str, ...] = (
    ...
    "AUDCAD", "AUDCHF", "AUDNZD", "CADCHF", "CADJPY", "CHFJPY", "GBPNZD", "NZDCAD", "NZDJPY",
    # High-volume equities
    "AAPL", "MSFT", "NVDA", "AMZN", "META", "TSLA", "GOOGL", "AVGO", "JPM", "AMD",
    # Indices
    "NAS100", "US500", "US30", "DAX40", "UK100", "JPN225", "HK50", "FRA40", "AUS200", "EU50", "DXY",
    # Metals
...
PROVIDER_ALIASES: Mapping[str, Mapping[str, tuple[str, ...]]] = {
    "canonical": {
        ...
        "DXY": ("DXY", "USDX", "DX-Y.NYB", "DOLLARINDEX"),
    },
    "twelve": {
...
def selected_symbols(state: Mapping[str, Any]) -> list[str]:
    ...
    main = main_symbol(state)
    if main not in selected:
        selected.insert(0, main)
    return selected or [main]


def main_symbol(state: Mapping[str, Any]) -> str:
    """Return the Settings-owned symbol used by Fields 4–9 and AI.

    The Lunch display symbol is intentionally ignored.  This prevents a fast
    Field 1–3 symbol switch from silently changing the production symbol for
    Fields 4–9.
    """
    registry = state.get("generation_registry_active_20260702")
    ...
    return candidate if candidate in SUPPORTED_SYMBOLS else "EURUSD"


def ensure_main_symbol_active(state: MutableMapping[str, Any]) -> dict[str, Any]:
    """Restore the saved main-symbol generation only when another symbol is active."""
    target = main_symbol(state)
    canonical = _canonical(state)
    current = normalize_symbol(canonical.get("symbol") or state.get("symbol") or target)
    state[MAIN_SYMBOL_KEY] = target
    if current == target:
        state[ACTIVE_KEY] = target
        return {"ok": True, "status": "ALREADY_ACTIVE", "symbol": target}
    report = activate_symbol_result(state, target)
    return report
...
def _synchronize_display_authorities(state: MutableMapping[str, Any], symbol: str) -> None:
    """Synchronize Lunch renderers without changing Settings/connector authority."""
    canonical = normalize_symbol(symbol)
    state[DISPLAY_SYMBOL_KEY] = canonical
    state[ACTIVE_KEY] = canonical
    state["active_snapshot_symbol_20260702"] = canonical
    state["canonical_display_symbol_20260705"] = canonical
    state["field1_display_symbol_20260706"] = canonical
    state["field2_display_symbol_20260706"] = canonical
    state["field3_display_symbol_20260706"] = canonical
    state["field10_display_symbol_20260706"] = canonical
    state["field11_display_symbol_20260706"] = canonical
    state["field11_active_symbol_20260705"] = canonical
    state["field11_symbol_sync_pending_20260702"] = canonical


def activate_symbol_view(state: MutableMapping[str, Any], symbol: Any) -> dict[str, Any]:
    """Atomically restore one complete published child snapshot.

    A persisted Field-10-only row is exposed only as an explicitly partial
    read-only state and never as a complete canonical Field 1/2 publication.
    """
    canonical = normalize_symbol(symbol)
    timeframe = str(state.get("selected_timeframe") or state.get("timeframe") or "H4").upper()
    publication: dict[str, Any] = {}
    try:
        from core.child_snapshot_publication_20260706 import latest_publication
        publication = latest_publication(symbol=canonical, timeframe=timeframe, db_path=DB_PATH, completed_only=True)
    except Exception:
        publication = {}

    if publication:
        report = activate_symbol_result(state, canonical)
        if report.get("ok"):
            try:
                from core.child_snapshot_publication_20260706 import validate_child_state, mark_reload_validation
                validation = validate_child_state(
                    state, runtime_snapshot_path=publication.get("runtime_snapshot_path"), db_path=DB_PATH,
                )
                exact = (
                    validation.get("identity", {}).get("symbol") == canonical
                    and str(validation.get("identity", {}).get("timeframe") or "").upper() == timeframe
                    and validation.get("identity", {}).get("snapshot_hash") == publication.get("snapshot_hash")
                )
                passed = bool(validation.get("ok") and exact)
                mark_reload_validation(publication=publication, db_path=DB_PATH, passed=passed, detail=validation)
                if passed:
                    _synchronize_display_authorities(state, canonical)
                    state.pop("lunch_symbol_readonly_row_20260705", None)
                    return {**report, "status": "COMPLETE_CHILD_RESTORED", "full_runtime_restored": True, "publication": publication, "validation": validation}
                report = {"ok": False, "status": "CHILD_RELOAD_VALIDATION_FAILED", "symbol": canonical, "validation": validation}
            except Exception as exc:
                report = {"ok": False, "status": "CHILD_RELOAD_VALIDATION_FAILED", "symbol": canonical, "error": f"{type(exc).__name__}: {exc}"}
    else:
        report = {"ok": False, "status": "NO_COMPLETE_CHILD_PUBLICATION", "symbol": canonical}

    # Database metadata can be missing after an older deployment, a database
    # reset, or a publish-then-rerun interruption even though the exact-symbol
    # runtime cache is complete.  Reuse the same strict child validator against
    # that cache before falling back to a Field-10-only row.  This remains a
    # read-only selector action: no provider call and no calculation is started.
    cache_path = _resolved_cache_path(canonical)
    if not publication and cache_path.is_file():
        previous_symbol = _canonical_symbol_from_state(state)
        runtime_report = activate_symbol_result(state, canonical)
        if runtime_report.get("ok"):
            try:
                from core.child_snapshot_publication_20260706 import validate_child_state
                # Legacy static contract: validate_child_state(state, runtime_snapshot_path=cache_path)
                validation = validate_child_state(state, runtime_snapshot_path=cache_path, db_path=DB_PATH)
                identity = validation.get("identity") if isinstance(validation.get("identity"), Mapping) else {}
                exact = (
                    str(identity.get("symbol") or "").upper() == canonical
                    and str(identity.get("timeframe") or "").upper() == timeframe
                )
                if validation.get("ok") and exact:
                    _synchronize_display_authorities(state, canonical)
                    state.pop("lunch_symbol_readonly_row_20260705", None)
                    return {
                        **runtime_report,
                        "status": "COMPLETE_RUNTIME_RESTORED",
                        "full_runtime_restored": True,
                        "runtime_cache_fallback": True,
                        "validation": validation,
                    }
                report = {
                    "ok": False, "status": "RUNTIME_CACHE_VALIDATION_FAILED",
                    "symbol": canonical, "validation": validation,
                }
            except Exception as exc:
                report = {
                    "ok": False, "status": "RUNTIME_CACHE_VALIDATION_FAILED",
                    "symbol": canonical, "error": f"{type(exc).__name__}: {exc}",
                }
        # A rejected cache must not leave Fields 1/2 pointed at a partial child.
        if previous_symbol and previous_symbol != canonical:
            with suppress(Exception):
                activate_symbol_result(state, previous_symbol)
                _synchronize_display_authorities(state, previous_symbol)

    row = _published_symbol_row(canonical)
    if not row:
        return report
    _synchronize_display_authorities(state, canonical)
    state["lunch_symbol_readonly_row_20260705"] = row
    return {
        "ok": True, "status": "PERSISTED_FIELD10_VIEW_READY", "symbol": canonical,
        "full_runtime_restored": False, "partial_display_only": True, "persisted_row": row,
        "message": "Only the persisted Field 10 row is available. Field 1 and Field 2 remain unavailable until a complete child publication passes reload validation.",
    }
...
def _run_selected_symbols_impl(
    ...
    # The ordered Settings selection is the primary authority.  A stale saved
    # MAIN_SYMBOL_KEY must never move an older symbol ahead of the user's first
    # current choice when the calculation transaction begins.
    selected = normalize_selected(state.get(SELECTED_KEY))
    if not selected:
        fallback_main = main_symbol(state)
        selected = [fallback_main] if fallback_main else []
    if not selected:
    ...
    requested_initial_display = normalize_symbol(
        state.get(DISPLAY_SYMBOL_KEY) or state.get(ACTIVE_KEY) or main
    )
    if main not in selected:
        selected.insert(0, main)
    ...
    for index, symbol in enumerate(selected, start=1):
        ...
        try:
            ...
            if not cached.get("ok"):
                clear_active_symbol_results(state)
                state["symbol"] = symbol  # legacy calculation mirror only
            state["calculation_symbol_20260702"] = symbol
            ...
            # Keep the additive lifecycle monitor as shadow evidence. It is not
            # used to replace protected production decisions.
            try:
                from core.canonical_runtime_20260617 import get_canonical
            ...
            state.pop("calculation_symbol_20260702", None)

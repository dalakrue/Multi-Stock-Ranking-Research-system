...
_PREFIXES = (
    ...
    "field6_", "field10_", "field11_", "field12_", "field13_", "field7_", "field8_", "field9_", "regime_", "powerbi_",
    ...
    try:
        ...
        for key, value in cached_state.items():
            ...
            if key in state and state.get(key) not in (None, "", False):
                ...
                if key not in _EXACT_KEYS and not str(key).startswith(("field10_", "field12_", "field13_")):
                    continue
            state[key] = value
            restored += 1
        ...
        if registry_row:
            state["multi_symbol_main_symbol_20260702"] = registry_row.get("settings_main_symbol") or state.get("multi_symbol_main_symbol_20260702") or "EURUSD"
            state["multi_symbol_selected_20260701"] = list(registry_row.get("selected_symbols") or [registry_row.get("symbol")])
            ...
            state["connector_symbol_20260702"] = registry_row.get("connector_symbol") or state.get("multi_symbol_main_symbol_20260702")
